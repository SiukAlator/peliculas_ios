...
//Rare exceptions to indicate a bug in the _prueba_ios code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString *)qualifiedName withProperties:(NSDictionary *)properties inContext:(id<TiEvaluator>)context
{
  ...
  if (proxyClass == nil) {
    NSString *_prueba_ios = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium."];
    if ([qualifiedName hasPrefix:_prueba_ios]) {
    ...
    if (proxyClass == nil) {
      ...
      @throw [NSException exceptionWithName:@"org.prueba_ios.module"
