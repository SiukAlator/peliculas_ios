...
static NSString *const kprueba_iosJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_prueba_iosInjection
{
  ...
  [html appendString:kprueba_iosJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
  ...
  // attempt to make well-formed HTML and inject in our prueba_ios bridge code
             ...
             baseURL:(NSURL *)baseURL
{
  ...
  content = [[self class] content:content withInjection:[self _prueba_iosInjection]];
            ...
            mimeType:(NSString *)mimeType
{
  ...
  [request setValue:[NSString stringWithFormat:@"%lu", (localId++)] forHTTPHeaderField:@"X-prueba_ios-Local-Id"];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
  ...
  if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
    ...
    if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
      [LocalProtocolHandler setContentInjection:[self _prueba_iosInjection]];
    }
    return YES;
  }
